...
```{r, cache = TRUE}
#Initializing Lists to be filled for mu = 0 (Will be used to add to dataframe later for plotting)
mu = 0
...
#Iterate through the 5000 dataframes of n = 30 for mu = 0
for (i in 1:5000) {
  
  #take a sample of 30 using mu = 0 and sd = 5
  sample = rnorm(n, mean = mu, sd = sigma)
  #take ttest + tidy to extract estimate and pvalue
  ...
  #fill lists
  reject_zero_list[[i]] = ifelse(t_test$p.value < 0.05, 1,0) #fills with 1 or 0 depending on reject
  mean_zero_list[[i]] = t_test$estimate #fill with estimate to take mean of 
  reject_mean_zero_list[[i]] = ifelse(t_test$p.value < 0.05, as.numeric(t_test$estimate),NA) # fill with mean if the null hypothesis is rejected, otherwise NA
}
reject_zero = mean(reject_zero_list) #find proportion rejected
est_mean_zero = mean(mean_zero_list) #find mean of mean estimates
est_mean_reject_zero = mean(reject_mean_zero_list, na.rm = TRUE) #find mean of mean estimates that were rejected 
```
...
```{r, cache = TRUE}
#Initiate list
ttest_list_mu = list() #Create nested list for the 6 additional mu x 5000 dataframes
true_mu = c(1:6)

#Iterate through 6 mus'
for (i in true_mu) {
  #Create temporary list to fill with 5000 dataframes
  ttest_list_mu[[i]] = list()
  
  #Iterate throuhg 5000 (Same as above)
  for (j in 1:5000) {
  ...
  #Record 5000 dataframes into ith mu
...
```{r}
#Create vector to fill with the 6 mean estimates
mean_est = vector("numeric", 6)

#Iterate through 6 means
for (i in true_mu) {
  #Create temp list to record means
  mean_vals = vector("numeric", 5000)
  
  #Iterate through the 5000 estimate means 
  for (j in 1:5000) {
    #... and record them
    mean_vals[[j]] = as.numeric(ttest_list_mu[[i]][[j]][1])
  }
  #Take the mean of the recorded estimate means
...
```{r}
#Create dataframe to plot estimate mean vs. true mean
est_true_df = tibble(
  mean = true_mu,
...
```{r}
#Create vector to fill with the 6 mean estimates that are rejected 
...
for (i in true_mu) {
  ...
  for (j in 1:5000) {
    #Only include mean estiamtes that successfully rejected null hypothesis, otherwise null
    mean_rej[j] = ifelse(ttest_list_mu[[i]][[j]][2] < 0.05, as.numeric(ttest_list_mu[[i]][[j]][1]), NA)
  }
  #record the mean of the recorded estimate means
...
```
* Black = Mean of samples that rejected null; Red = Mean of all samples.
